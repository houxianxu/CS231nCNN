...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    # tile_X_train = np.tile(self.X_train, (num_test, 1)) # [num_test*num_train, ~]
    ...
    X_Y = X.dot(self.X_train.T); # [num_test, num_attr] * [num_attr, num_train]
    square_sum_tr = np.sum(np.square(self.X_train), axis=1) # [1, num_train]
    square_sum_te = np.sum(np.square(X), axis=1) # [1, num_test]

    dists = (-2 * X_Y) + square_sum_tr + np.matrix(square_sum_te).T
    #########################################################################
    #                         END OF YOUR CODE                              #
    #########################################################################
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      k_nearest_neighbor = np.argsort(dists[i])[0:k]
